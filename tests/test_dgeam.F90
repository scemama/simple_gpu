program test_dgeam
  ...
  print *, "================================"
  print *, "DGEAM (matrix addition) Tests"
  print *, "================================"
  print *, ""
  ...
  alpha = 1.0d0
  beta = 1.0d0
  call gpu_dgeam(handle, 'N', 'N', m, n, alpha, a%f(1,1), m, beta, b%f(1,1), m, c%f(1,1), m)
  ...
  if (maxval(abs(c_h - c_expected)) < tol * (abs(alpha) + abs(beta))) then
    print *, "  PASSED"
    test_passed = test_passed + 1
  else
    print *, "  FAILED - Max error:", maxval(abs(c_h - c_expected))
  ...
  alpha = 1.0d0
  beta = 1.0d0
  call gpu_dgeam(handle, 'T', 'N', m, n, alpha, a%f(1,1), n, beta, b%f(1,1), m, c%f(1,1), m)
  call gpu_download(c, c_h)
  ...
  alpha = 1.0d0
  beta = 1.0d0
  call gpu_dgeam(handle, 'N', 'T', m, n, alpha, a%f(1,1), m, beta, b%f(1,1), n, c%f(1,1), m)
  call gpu_download(c, c_h)
  ...
  ! =========================================
  ! Test 5: C = A^T + B^T (transpose both)
  ! =========================================
  print *, ""
  print *, "Test 5: C = A^T + B^T (T, T, alpha=1, beta=1)"
  total_tests = total_tests + 1
  ...
  deallocate(a_h, b_h, c_h, c_expected)
  allocate(a_h(n,m), b_h(n,m), c_h(m,n), c_expected(m,n))
  ...
  alpha = 1.0d0
  beta = 1.0d0
  call gpu_dgeam(handle, 'T', 'T', m, n, alpha, a%f(1,1), n, beta, b%f(1,1), n, c%f(1,1), m)
  call gpu_download(c, c_h)

  c_expected = transpose(a_h) + transpose(b_h)
