program test_sdot
  ...
  call gpu_sdot(handle, n, x%f(1), 1, y%f(1), 1, result)
  ...
  call gpu_sdot(handle, n/2, x%f(1), 2, y%f(1), 2, result)
  ...
  expected = sum(x_h * y_h)
  if (abs(result - expected) < tol * n * n * n) then
    print *, "  PASSED - Result:", result
    test_passed = test_passed + 1
  else
    print *, "  FAILED - Expected:", expected, "Got:", result
